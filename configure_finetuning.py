...
class FinetuningConfig(object):
  ...
  def __init__(self, model_name, data_dir, **kwargs):
    ...
    self.vocab_size = 30  # number of tokens in the vocabulary
    self.do_lower_case = False # originally True

    # training
    self.learning_rate = 5e-6 # originally 1e-4
    ...
    self.num_train_epochs = 5.0    # 3  passes over the dataset during training
    ...
    # for qa tasks
    self.max_query_length = 512   # 128 max tokens in q as opposed to context
    self.doc_stride = 128  # 128 stride when splitting doc into multiple examples
