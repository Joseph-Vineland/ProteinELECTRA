...
class fluorescence(RegressionTask):
  """Protein Fluoresence Regression"""

  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(fluorescence, self).__init__(config, "fluorescence", tokenizer, 1.0, 4.5)
...
class stability(RegressionTask):
  """Protein Stability Regression"""

  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(stability, self).__init__(config, "stability", tokenizer, -2.0, 3.5)
...
class remoteHomology(ClassificationTask):
  """Protein Fold Classification"""

  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(remoteHomology, self).__init__(config, "remoteHomology", tokenizer, list(map(str,list(range(1195)))) )

  def _create_examples(self, lines, split):
    return self._load_glue(lines, split, 0, None, 1, True)


class promoters(ClassificationTask):
  """Promoters Classification"""

  def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
    super(promoters, self).__init__(config, "promoters", tokenizer, ["0", "1"] )
